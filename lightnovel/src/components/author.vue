      ...
      <div class="am-body" v-for="item in novelData" :key="item.id">
        <div class="am-item">
          <a target="_blank" :href="/book/ + item.id"
            ><img class="am-item-img" :src="item.novel_url"
          /></a>
          ...
          <div class="am-item-div">
            ...
            <div class="am-right-num">
              <span class="am-right-num-span">{{ item.word_nums }}万字</span
            ...
            <div class="am-right-bot">
              <a target="_blank" class="am-right-read" :href="item.novel_paths"
                >立即阅读</a
              >
              <div class="am-right-shelf js-addShelf" data-bid="7098680">
                +书架
              </div>
            </div>
          </div>
  ...
  methods: {
    async getNovelData() {
      ...
      const { data: data1 } = await this.$http.get(
        `/novels?keyword=${data.data[0].novel_author}`
      );
